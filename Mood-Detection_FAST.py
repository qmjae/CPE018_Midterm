...
def read_images(path):
    ...
    fast = cv2.FastFeatureDetector_create()  # Initialize FAST feature detector
        ...
        for subdirname in dirnames:
            ...
            for filename in os.listdir(subject_path):
                try:
                    ...
                    keypoints = fast.detect(resized_im, None)  # Detect FAST keypoints

                    if keypoints is not None and len(keypoints) > 0:
                        # Extract descriptors using BRIEF (Binary Robust Independent Elementary Features)
                        brief = cv2.xfeatures2d.BriefDescriptorExtractor_create()
                        _, descriptors = brief.compute(resized_im, keypoints)
                        ...
                        print(f"No keypoints found for image: {filepath}")
